...
>[!note]
>
> >**例1: 设 $f(x,y) = (x^2 + y^2)  \displaystyle\sin \displaystyle\frac{1}{x^2 + y^2}$,求证：$ \displaystyle\lim_{(x,y) \to (0,0)} f(x,y) = 0.$**
> > 
...
>
> >**例2: 考察函数在 $(0, 0)$ 处的极限, $$ f(x,y) = \begin{cases} \displaystyle\displaystyle\frac{xy}{x^2 + y^2}, & x^2 + y^2 \neq 0, \\0, & x^2 + y^2 = 0.\end{cases} $$.**
> > 
> >**解:**显然,当点 $P(x,y)$ 沿 $x$ 轴趋于点 $(0,0)$ 时,
> >$$  \displaystyle\lim_{(x,y) \to (0,0)} f(x,y) =  \displaystyle\lim_{x \to 0} f(x,0) =  \displaystyle\lim_{x \to 0} 0 = 0; $$
> >又当点 $P(x,y)$ 沿 $y$ 轴趋于点 $(0,0)$ 时,
> >$$  \displaystyle\lim_{(x,y) \to (0,0)} f(x,y) = \displaystyle\lim_{y \to 0} f(0,y) =  \displaystyle\lim_{y \to 0} 0 = 0. $$
> >虽然点 $P(x,y)$ 以上述两种特殊方式（沿 $x$ 轴或沿 $y$ 轴）趋于原点时函数的极限存在且相等,但是 $ \displaystyle\lim_{(x,y) \to (0,0)} f(x,y)$ 并不存在,这是因为当点 $P(x,y)$ 沿着直线 $y = kx$ 趋于点 $(0,0)$ 时,有
> >$$  \displaystyle\lim_{(x,y) \to (0,0)} \displaystyle\frac{xy}{x^2 + y^2} =  \displaystyle\lim_{x \to 0} \displaystyle\frac{kx^2}{x^2 + k^2 x^2} = \displaystyle\frac{k}{1+k^2}, $$
> >显然它是随着 $k$ 的值的不同而改变的,
> 
>
>>**例3: 求 $  \displaystyle\lim_{(x,y) \to (0,2)} \displaystyle\frac{\sin(xy)}{x} $.**
> >
> >**解:** 这里函数 $ \displaystyle\frac{\sin(xy)}{x} $ 的定义域为 $D = \{(x,y) | x \neq 0, y \in \mathbb{R}\}$,$P_0(0,2)$ 为 $D$ 的聚点,由积的极限运算法则,得
...
> $$ \left. \displaystyle\frac{\partial z}{\partial x} \right|_{x=x_0}, \quad \left. \displaystyle\frac{\partial f}{\partial x} \right|_{x=x_0}, \quad z_x \bigg|_{x=x_0} \text{ 或 } f_x(x_0, y_0) \tag{1} $$
> **示例表达式**  
> 极限 $(2-1)$ 可表示为：  
> $$ f_x(x_0, y_0) =  \displaystyle\lim_{\Delta x \to 0} \displaystyle\frac{f(x_0 + \Delta x, y_0) - f(x_0, y_0)}{\Delta x} \tag{2-2} $$ 
> 类似地,定义对 $y$ 的偏导数为：  
> $$  \displaystyle\lim_{\Delta y \to 0} \displaystyle\frac{f(x_0, y_0 + \Delta y) - f(x_0, y_0)}{\Delta y} \tag{2-3} $$  
> 记作：  
...
> >**解:**
> > **计算 $f_x(0, 0)$**  
> > 根据偏导数定义：$ f_x(0, 0) =  \displaystyle\lim_{\Delta x \to 0} \displaystyle\frac{f(0+\Delta x, 0) - f(0, 0)}{\Delta x} =  \displaystyle\lim_{\Delta x \to 0} \displaystyle\frac{0 - 0}{\Delta x} = 0$
> > **计算 $f_y(0, 0)$**  
> >同理：$f_y(0, 0) =  \displaystyle\lim_{\Delta y \to 0} \displaystyle\frac{f(0, 0+\Delta y) - f(0, 0)}{\Delta y} =  \displaystyle\lim_{\Delta y \to 0} \displaystyle\frac{0 - 0}{\Delta y} = 0$
> >即$ f_x(0, 0) = 0, \quad f_y(0, 0) = 0 $

>[!important]
>**高阶偏导数**
>设函数 $z = f(x, y)$ 在区域 $D$ 内具有偏导数
>$$ \displaystyle\displaystyle\frac{\partial z}{\partial x} = f_x(x, y), \quad \displaystyle\displaystyle\frac{\partial z}{\partial y} = f_y(x, y) $$
...
> >结果验证  
> >观察到混合偏导数相等：
> >$$ \displaystyle\frac{\partial^2 z}{\partial y \partial x} = \displaystyle\frac{\partial^2 z}{\partial x \partial y} = 6x^2 y - 9y^2 - 1 $$
> >符合Schwarz定理（当二阶混合偏导数连续时,求导顺序可交换）

>[!important]
>**二阶混合偏导数定理**  
> 如果函数 $z = f(x, y)$ 的两个二阶混合偏导数  $ \displaystyle\frac{\partial^2 z}{\partial y \partial x} \quad \text{和} \quad \displaystyle\frac{\partial^2 z}{\partial x \partial y} $  
> 在区域 $D$ 内连续,那么在该区域内必有： $ \displaystyle\frac{\partial^2 z}{\partial y \partial x} = \displaystyle\frac{\partial^2 z}{\partial x \partial y} $
> 即：**二阶混合偏导数在连续条件下与求导次序无关**,  

>[!note]
>
> >**例1:验证函数 $z = \ln \sqrt{x^2 + y^2}$ 满足 ==拉普拉斯方程== ：$$ \displaystyle\frac{\partial^2 z}{\partial x^2} + \displaystyle\frac{\partial^2 z}{\partial y^2} = 0 .$$**
> >
> >**证明:**   
> >首先将函数化简为：
...
>
>
> >**例2:证明函数 $u = \dfrac{1}{r}$ 满足==拉普拉斯方程==：$$ \displaystyle\frac{\partial^2 u}{\partial x^2} + \displaystyle\frac{\partial^2 u}{\partial y^2} + \displaystyle\frac{\partial^2 u}{\partial z^2} = 0 $$,其中 $r = \sqrt{x^2 + y^2 + z^2}$.**
> >
> >**证明:**
...
> >- $\displaystyle\frac{\partial z}{\partial x}$：$x$方向的变化率
> >- $\displaystyle\frac{\partial z}{\partial y}$：$y$方向的变化率
> >- $dx, dy$：自变量的微小变化量
...
> 1. 该函数在点 $(x,y)$ 的偏导数 $\displaystyle\frac{\partial z}{\partial x}$ 和 $\displaystyle\frac{\partial z}{\partial y}$ 必定存在
> 2. 函数在点 $(x,y)$ 的全微分为
> $$ dz = \displaystyle\frac{\partial z}{\partial x} \Delta x + \displaystyle\frac{\partial z}{\partial y} \Delta y \tag{3-1} $$
> >**证明:**  
...
> 两边除以 $\Delta x$ 并取极限：
> $$  \displaystyle\lim_{\Delta x\to 0}\displaystyle\frac{f(x+\Delta x, y)-f(x,y)}{\Delta x} = A $$
> 由此可得 $\displaystyle\frac{\partial z}{\partial x} = A$,
> 同理可证  
> 取 $\Delta x=0$ 可证得 $\displaystyle\frac{\partial z}{\partial y} = B$,
> 因此全微分表达式 (3-1) 得证,
...
> >$$ f(x,y) = 
> >\begin{cases} 
> >\displaystyle\frac{xy}{\sqrt{x^2+y^2}}, & x^2+y^2 \neq 0 \\
> >0, & x^2+y^2=0 
> >\end{cases} $$.**
> >
> >**证:**
> >在点 $(0,0)$ 处：$ f_x(0,0)=0,\quad f_y(0,0)=0 $
> >计算增量差：$ \Delta z - [f_x(0,0)\Delta x + f_y(0,0)\Delta y] = \displaystyle\frac{\Delta x \Delta y}{\sqrt{(\Delta x)^2+(\Delta y)^2}} $
> >沿直线 $y=x$ 趋近时：$ \displaystyle\frac{\Delta x \Delta y}{(\Delta x)^2+(\Delta y)^2} \bigg|_{y=x} = \displaystyle\frac{(\Delta x)^2}{2(\Delta x)^2} = \displaystyle\frac{1}{2} \nrightarrow 0 $
> >结论:当 $\rho \to 0$ 时,$\Delta z - [f_x\Delta x + f_y\Delta y]$ 不是$\rho$的高阶无穷小,因此：$\text{函数在}(0,0)\text{处不可微分}$

> [!important]
> 
> **定理2（充分条件）**
> 如果函数 $z=f(x,y)$ 的偏导数 $\displaystyle\frac{\partial z}{\partial x}$ 和 $\displaystyle\frac{\partial z}{\partial y}$ 在点 $(x,y)$ 连续,那么函数在该点可微分,
>
> **证明:** 
> 设偏导数 $\displaystyle\frac{\partial z}{\partial x}$ 和 $\displaystyle\frac{\partial z}{\partial y}$ 在点 $P(x,y)$ 的某邻域内存在且连续,
...
> 其中 $ \displaystyle\lim_{(\Delta x,\Delta y)\to(0,0)} \varepsilon_1 = 0$
> 同理可得：
> $$ f(x,y+\Delta y) - f(x,y) = f_y(x,y)\Delta y + \varepsilon_2\Delta y \quad \text{(3-5)} $$
> 其中 $ \displaystyle\lim_{\Delta y\to 0} \varepsilon_2 = 0$
> 综合得：
> $$ \Delta z = f_x(x,y)\Delta x + f_y(x,y)\Delta y + \varepsilon_1\Delta x + \varepsilon_2\Delta y \quad \text{(3-6)} $$ 
> 由于：
> $$ \left| \displaystyle\frac{\varepsilon_1\Delta x + \varepsilon_2\Delta y}{\rho} \right| \leq |\varepsilon_1| + |\varepsilon_2| \to 0 $$
...
> $\overrightarrow{OA} = \begin{pmatrix} \Delta x \\ 0 \\ \displaystyle\frac{\partial f}{\partial x} \Delta x \end{pmatrix}$
> 向量 $\overrightarrow{OB}$ 表示沿 $y$ 方向的变化：
> $\overrightarrow{OB} = \begin{pmatrix} 0 \\ \Delta y \\ \displaystyle\frac{\partial f}{\partial y} \Delta y \end{pmatrix}$
> 合成向量 $\overrightarrow{OC}$：
>  $\overrightarrow{OC} = \overrightarrow{OA} + \overrightarrow{OB} = \begin{pmatrix} \Delta x \\ \Delta y \\ \displaystyle\frac{\partial f}{\partial x} \Delta x + \displaystyle\frac{\partial f}{\partial y} \Delta y \end{pmatrix}$
>全微分关系
> 函数增量表达式：$
>   f(x_0 + \Delta x, y_0 + \Delta y) = f(x_0, y_0) + \displaystyle\frac{\partial f}{\partial x} \Delta x + \displaystyle\frac{\partial f}{\partial y} \Delta y
>  $
>增量与微分关系：$
>   \Delta f = \displaystyle\frac{\partial f}{\partial x} \Delta x + \displaystyle\frac{\partial f}{\partial y} \Delta y
>   $
>全微分形式：$
>   df = \displaystyle\frac{\partial f}{\partial x} dx + \displaystyle\frac{\partial f}{\partial y} dy
>   $
>几何解释
...
>1、一元函数微分
>$
>df = \displaystyle\frac{\partial f}{\partial x} \cdot dx = f' \cdot dx
>$
>$
>f = f_0 + f' \cdot dx
>$
...
>$
>$
>= \left( \displaystyle\frac{\partial f}{\partial x}, \displaystyle\frac{\partial f}{\partial y} \right)^T \cdot \left( dx, dy \right)^T
>$
>$
...
> 3、三元函数扩展
>$
>df = \left( \displaystyle\frac{\partial f}{\partial x}, \displaystyle\frac{\partial f}{\partial y}, \displaystyle\frac{\partial f}{\partial z} \right)^T \cdot \left( dx, dy, dz \right)^T
>$
>$
>= \displaystyle\frac{\partial f}{\partial x} dx + \displaystyle\frac{\partial f}{\partial y} dy + \displaystyle\frac{\partial f}{\partial z} dz
>$
>$
...
> >- 满足 $z_0 = f(x_0,y_0)$
> >- 其偏导数为：
> >$$ \displaystyle\frac{\partial z}{\partial x} = -\displaystyle\frac{F_x}{F_z}, \quad \displaystyle\frac{\partial z}{\partial y} = -\displaystyle\frac{F_y}{F_z} \tag{5-4} $$
> >**公式推导**  
...
> > **记忆口诀**
> > "交叉偏导,负号相连"
> > - 分子：非目标变量的偏导（如求$\displaystyle\frac{\partial z}{\partial x}$时分子是$\displaystyle\frac{\partial F}{\partial x}$）
> > - 分母：目标变量分母的偏导（总是$\displaystyle\frac{\partial F}{\partial z}$）
>
>
